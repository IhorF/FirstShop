...
<div style="float:left margin: 0;"></div>
	...
	<div style="border: solid 2px grey; width: 20%;float: left; ">
		...
		<div >
			<h2 style="color: red; text-align: center">Articul: ${commodity.articul}</h2>
		</div>
<br><br>
			<a>Characteristics:</a> <br>${commodity.composition}<br>${commodity.category}<br>${commodity.gender}<br>${commodity.size}<br>Color:
			...
			<sec:authorize access="hasRole('ROLE_USER')">
				<form action="buyCommodity/${commodity.id}" class="buy">
					...
					<input type="text" id="cost" style="width: 40px" name="quantity"
						value="1" pattern="[0-9]{1,10000}" title="Only numbers!" maxlength="5">
					<button>buy</button>
				</form>
