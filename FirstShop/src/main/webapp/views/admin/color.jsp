...
<div>
	...
	<form:form modelAttribute="color" action="addColor" method="post">
		...
		<form:input path="name" placeholder="color" pattern="[a-z]{3,}"
			title="Only letters!" maxlength="15" />
		<button title="Save color">save color</button>
		<br>
		<br>
		<br>
		<a>Existing colors:</a>
	...
	${c.count}. ${color.name} 
	<a href="deleteColor/${color.id}" id="colorId" name="${color.id}">delete</a>
			<br>
		</c:forEach>
	</form:form>
</div>